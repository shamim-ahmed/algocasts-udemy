...
describe('A Node', () => {
  test('has properties "data", "previous" and "next"', () => {
    const node = new Node('a', 'b', 'c');
    expect(node.data).toEqual('a');
    expect(node.previous).toEqual('b');
...
describe('GetLast', () => {
  test('returns the last element', () => {
    const l = new List();
    l.insertFirst(2);
    expect(l.getLast()).toEqual({ data: 2, previous: null, next: null });
    l.insertFirst(1);
    expect(l.getLast()).toEqual({ data: 2, previous: l.getFirst(), next: null });
  });
});
